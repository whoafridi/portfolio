...
import type { DefaultImageParser } from './utils/getConfig'
...
const exportableLoader: ImageLoader = ({ src: _src, width, quality }) => {
  ...
  if (config.basePath !== undefined) src = _src.replace(config.basePath, '')
  ...
  if (
    ...
    process.env['TEST_JSON_PATH'] !== undefined
  ) {
...
export default exportableLoader
